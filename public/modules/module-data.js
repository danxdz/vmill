(() => {
  ...
  function normalizeImportedAppState(v) {
    ...
    if (Array.isArray(st.stations) && st.stations.length) {
      const fallbackStationId = String(st.stations[0]?.id || "");
  ...
  function readWindowAppState() {
    ...
    return isValidAppState(store?.app) ? store.app : null;
  }
  ...
  function readAppState(options = {}) {
    ...
    const candidates = [];
    for (const key of ALL_APP_KEYS) {
      const parsed = readRawJsonByKey(key);
    ...
    if (isValidAppState(fromWindow)) candidates.push({ key: "__window_name__", state: fromWindow, stamp: stateStamp(fromWindow) });
  ...
  function writeAppState(next) {
    const normalized = ensureStateMeta(next);
